...
expression_t *expression_variable(variable_t *var, scope_t *scope)
{
	...
	for ( parent=NULL, p = var; p; parent=p, p = p->child ) {
		if ( p->id ) {
			/*
			...
			if ( parent == NULL ) {
				...
				if ( p->child ) { // this is supposed to be a Record
					if ( p->type.dataType != VT_User ) {
						printf( "%d: %s Is not a record (%d)\n", yylineno, p->id, p->type.dataType);
						return 0;
					}

					type = st_typedef_find(p->type.userType, scope );
					...
					switch ( type->type ) {
						...
						case TT_Array:
							if ( type->array.typename.dataType != VT_User ) {
								printf("%d: Array should have been user defined (a record)!\n"
										, yylineno);
								return NULL;
							}

							typedefs_entry_t *rec;

							rec = st_typedef_find(type->array.typename.userType, scope);

							if ( rec == NULL ) {
								printf("%d: Custom %s is not defined\n",
										yylineno,type->array.typename.userType);

								return NULL;
							}

							if ( rec->type != TT_Record ) {
								printf("%d: %s should have been a record\n",
										yylineno, type->array.typename.userType);
							}
						...
						default:
							printf("%d: [%s] Typedef %s is not a record/array\n",
			...
			if ( parent->type.dataType != VT_User ) {
				printf( "%d: %s Is not custom (%d:%s)\n", 
						yylineno, parent->id, parent->type.dataType, parent->id);
				return NULL;
			}
			...
			found = 0;
			for ( i = 0; i < record->size && !found; i ++ )
				if ( !strcasecmp(record->ids[i], p->id) ) {
					found = 1;
					p->type.dataType = record->types[i].dataType;
					p->type.userType = record->types[i].userType;
					variableType = record->types[i].dataType;
				}
			...
			if ( type->array.dims.size != p->expr.size ) {
				printf("%d: Given limits (%d) do not match the"
						" defined dimensions of the array (%d).\n",
